...
# Hard cap on how long to wait for a task to leave PROCESSING. A task that never finishes
# (Open Cloud incident, stuck queue) would otherwise spin this loop forever — in CI that holds
# the luau-execution concurrency lock until GitHub's 6-hour job kill. The test suite normally
# completes in a few minutes; 20 minutes is generous headroom for upload/queue delays.
TASK_COMPLETION_TIMEOUT_SECONDS = 20 * 60
...
def pollForTaskCompletion(api_key, path):
    ...
    deadline = time.monotonic() + TASK_COMPLETION_TIMEOUT_SECONDS
    while True:
        try:
        ...
        if task['state'] != 'PROCESSING':
            ...
            sys.stderr.flush()
            logging.error(
                f'Task did not finish within {TASK_COMPLETION_TIMEOUT_SECONDS} seconds '
                f'(still PROCESSING), giving up: {path}')
            sys.exit(1)
        else:
            sys.stderr.write('.')
            sys.stderr.flush()
...
def getTaskLogs(api_key, task_path):
    ...
    # A task that produced no output returns an empty (or absent) log-group list; index [0]
    # unconditionally would raise IndexError and surface as an opaque CI traceback. Treat it as
    # "no logs" instead, matching handleLogs' empty-string handling.
    groups = logs.get('luauExecutionSessionTaskLogs') or []
    if not groups:
        return ''
    messages = groups[0].get('messages', [])
    return ''.join([m + '\n' for m in messages])
